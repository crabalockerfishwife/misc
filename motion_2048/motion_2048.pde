int[][] b = new int[4][4];
int[][] bak = new int[4][4];
int[][] prev[] = new int[4][4][3];
int pad = 20;
int bs = 100;
int len = pad*(b.length+1)+bs*b.length;
int score = 0;
int highscore = 0;
int animStart;
int animLength = 10;
boolean dead = true;
PFont font, scorefont, endfont;
...
void setup() {
  ...
  scorefont = loadFont("Arial-BoldMT-10.vlw");
  endfont = loadFont("Arial-BoldMT-30.vlw");
  textFont(font);
}
...
void draw() {
  ...
  text("Highscore: "+highscore, 150, 15);
  textFont(font);
  if(dead) {
    fill(255,100);
